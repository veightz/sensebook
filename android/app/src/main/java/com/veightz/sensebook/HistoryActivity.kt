...
class HistoryActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        button("开启可选同步") { val raw = config.text.toString(); val old = include.isChecked; work { QuerySync.connect(applicationContext,raw,old); QuerySync.sync(applicationContext) }; config.text.clear() }
        button("立即同步 / 重试") { work { QuerySync.sync(applicationContext) } }
        button("关闭同步") { work { QuerySync.disable(applicationContext) } }
