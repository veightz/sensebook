 ...
 *        node scripts/harness-fab.mjs --legacy-fab-pos
 */
import fs from 'fs';
...
const legacyFab = process.argv.includes('--legacy-fab-pos');
...
if (legacyFab) store.set('sensebook_fab_pos', { left: 400, top: 300 });
...
const right = fab.style && fab.style.right;
const bottom = fab.style && fab.style.bottom;
const left = fab.style && fab.style.left;
const top = fab.style && fab.style.top;
const attrStyle = fab.attrs && fab.attrs.style;
const positioned =
  (right && bottom) ||
  (attrStyle && /right\s*:/.test(attrStyle) && /bottom\s*:/.test(attrStyle));

if (!positioned) {
  console.error('FAIL: expected right/bottom position, got', { right, bottom, left, top, attrStyle, style: fab.style });
  process.exit(1);
}

if (legacyFab) {
  const cleared = !store.has('sensebook_fab_pos') || store.get('sensebook_fab_pos') == null;
  if (!cleared) {
    console.error('FAIL: legacy left/top should be cleared from store, got', store.get('sensebook_fab_pos'));
    process.exit(1);
  }
}
...
console.log(
  'PASS: #sensebook-fab-root present',
  throwMenu ? '(GM menu threw)' : '(GM menu ok)',
  nullStyle ? '(null style)' : '',
  corruptFab ? '(corrupt fab pos)' : '',
  legacyFab ? '(legacy left/top ignored)' : '',
);
console.log('  right=', right, 'bottom=', bottom, 'attrStyle=', attrStyle ? attrStyle.slice(0, 80) : undefined);
